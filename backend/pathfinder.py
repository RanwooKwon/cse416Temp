    ...
    try:
        ...
        if lot_match.empty:
            ...
            if lot_match.empty:
                ...
                if lot_match.empty:
                    raise HTTPException(status_code=404, detail=f"Parking lot '{name}' not found in map data and no fallback available")
...
def find_nearest_available_lots(start_lat: float, start_lng: float, 
                               limit: int = 5, 
                               min_available: int = 1, 
                               prefer_ev: bool = False) -> List[Dict]:
    G_undirected, gdf_polygon = _load_road_graph()
    ...
    try:
        ...
        for lot in available_lots:
            ...
            try:
                try:
                    path = nx.astar_path(G_undirected, start_node, lot_node, weight="length")
                except KeyError:
                    path = nx.astar_path(G_undirected, start_node, lot_node, weight="weight")
        ...
        if prefer_ev:
            lots_with_paths.sort(key=lambda x: (-x["evSlots"], x["distance"]))
        else:
            lots_with_paths.sort(key=lambda x: x["distance"])
